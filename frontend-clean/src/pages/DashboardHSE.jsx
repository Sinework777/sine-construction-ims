...
const DashboardHSE = () => {
  ...
  useEffect(() => {
    axiosInstance.get('/hse').then(res => setHSE(res.data.hse)).catch(() => setHSE([]));
  }, []);
    ...
    <div className="flex">
      ...
      <main className="flex-1 ml-20 md:ml-64 p-6">
        <div className="w-full max-w-6xl mx-auto py-8 px-4 sm:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 mb-8">
            {hse.map(h => (
              <div key={h.id} className="bg-red-100 rounded-xl p-6 shadow flex flex-col items-center">
                <div className="font-bold text-lg">{h.incident}</div>
                <div className="text-sm text-gray-600">Resolved: {h.resolved ? 'Yes' : 'No'}</div>
              </div>
            ))}
          </div>
        </div>
