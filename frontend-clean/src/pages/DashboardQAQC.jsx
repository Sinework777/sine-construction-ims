...
const DashboardQAQC = () => {
  ...
  useEffect(() => {
    axiosInstance.get('/qaqc').then(res => setQAQC(res.data.qaqc)).catch(() => setQAQC([]));
  }, []);
          ...
          <main className={`pt-24 px-8 transition-all duration-300 ${collapsed ? 'ml-20' : 'ml-64'} max-w-7xl mx-auto`}>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 mb-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 mb-8">
                {qaqc.map(q => (
                  <div key={q.id} className="bg-yellow-100 rounded-xl p-6 shadow flex flex-col items-center">
                    <div className="font-bold text-lg">{q.issue}</div>
