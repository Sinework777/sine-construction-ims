...
export default function MinutesForm({ onSave, onCancel }) {
  ...
  function handleExport(type) {
    try {
      ...
      setError('Failed to export meeting minutes.');
    }
  }
  ...
  return (
    <>
      ...
      <form id="minutes-form-container" className="space-y-4 p-4 bg-white rounded-2xl shadow-md" onSubmit={e => { e.preventDefault(); try { onSave({ title, status, date }); setError(''); } catch { setError('Failed to save meeting minutes.'); } }}>
