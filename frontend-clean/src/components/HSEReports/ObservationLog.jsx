...
export default function ObservationLog({ userRole, onAdd }) {
  ...
  function handleSubmit(e) {
    e.preventDefault();
    if (userRole === 'viewer') {
      setError('You do not have permission to submit observations.');
      return;
    }
    try {
      ...
      {showForm && (
        <form className="bg-white p-4 rounded-xl shadow-xl grid grid-cols-1 md:grid-cols-2 gap-4 mb-4" onSubmit={handleSubmit} aria-label="Observation Entry Form">
          ...
          <div className="md:col-span-2 flex gap-4 justify-end mt-2">
            <button type="submit" className="bg-green-600 text-white px-4 py-2 rounded-xl shadow hover:bg-green-700 transition" disabled={userRole === 'viewer'} title={userRole === 'viewer' ? 'Viewers cannot submit' : 'Save'}>Save</button>
