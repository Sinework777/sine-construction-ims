...
export default function NCRForm({ onSave, onCancel }) {
  ...
  function handleSubmit(e) {
    e.preventDefault();
    if (!canEdit) {
      setError('You do not have permission to save NCRs.');
      return;
    }
    try {
      onSave({ title, status, date });
      setError('');
    } catch {
  ...
  return (
    <>
      ...
      <form id="ncr-form-container" className="space-y-4 p-4 bg-white rounded-2xl shadow-md" onSubmit={handleSubmit}>
