...
export default function DashboardHome({ user = { name: 'Admin User', role: 'Admin', project: 'Tower A', lastLogin: '2025-07-19T22:15:00Z', avatar: '' } }) {
  const [project, setProject] = useState(user.project);
  const [now, setNow] = useState(new Date());
  const [avatar, setAvatar] = useState(user.avatar || 'https://randomuser.me/api/portraits/men/32.jpg');
  ...
  const greeting = userRoles[user.role] || userRoles.Default;
