  ...
  function handleExport(type) {
    ...
    if (type === 'PDF') {
      html2canvas(viewElement).then(canvas => {
        ...
        const filename = `MaterialLogView_${log.deliveryDate || new Date().toISOString().split('T')[0]}.pdf`;
        pdf.save(filename);
      });
    } else if (type === 'Excel') {
      const data = [{
      ...
      const filename = `MaterialLogView_${log.deliveryDate || new Date().toISOString().split('T')[0]}.xlsx`;
